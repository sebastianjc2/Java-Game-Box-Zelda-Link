...
public class ZeldaMapMakerState extends State{
	...
	public int count=0,randSet,randTile;
	boolean genRand;
	...
	@Override
	public void tick() {
		if(handler.getKeyManager().keyJustPressed(KeyEvent.VK_R)) { //Press 'R' to pick a random tile 
		...
		if((handler.getKeyManager().shift && handler.getKeyManager().rKey) && !genRand) { //Activates when Shift + R are pressed, continues in next if...
			...
			handler.getKeyManager().rKey=false;
			genRand=true;
		}
		if(genRand) { // Continuation...Picks a random tile set, and random tile
			randSet = rand.nextInt(5);
			switch (randSet) {
			case 0:
				selectedList = Images.zeldaTiles;
				break;
			...
			randTile = rand.nextInt(selectedList.size());
			counter = randTile;
			genRand=false;
		}
